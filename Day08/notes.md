...
- 类是单继承，但是协议可以多遵守
    ...
    @end
    ```
- 协议可以从另一个协议继承，而且可以多继承
    - 协议可以继承另一个协议，如果A协议继承了B协议，那么A协议中不仅有自己的方法声明，还有B中的方法声明
    - 语法：
    @protocol A协议名称 <B协议名称>
    - NSObject：这是一个类，是所有OC对象的基类，这个类是苹果定义好的
    - NSObject：这也是一个协议，也是苹果定义好的，这个协议被NSObject类遵守，所以所有的OC对象都拥有这个协议中所有的方法，这个协议叫做基协议。
    - 协议的名称可以和类的名称相同。
- @required与@operational
    - 在协议中，如果方法的声明被@required修饰，那么遵守这个协议的类必须要实现这个方法，否则编译器会发出警告。
    - 在协议中，如果方法的声明被optional修饰，那么遵守协议的类可以实现也可以不实现，不实现编译器也不会警告。
    - 但其实不管用required还是operational修饰，类都可以选择实现或者不实现，不会影响编译及运行，唯一的区别是：当遵守协议的类不实现协议中的方法的时候，required会给一个警告，operation连警告都没有
    - 这两个关键字的主要作用：在于程序员沟通，告诉遵守协议的类，哪些方法是必须要实现的。（默认required）

- 写协议的规范：
    - 要求所有的协议都必须直接或者间接的从NSObject基协议继承。

- 协议的类型限制：
    - 要求声明一个指针，指针必须指向遵守A协议的对象，否则起码给一个警告
        ```objective-c
        NSObject<A> *obj = [Student new];
        id<A> *obj = [Student new];
        ```
    - 声明一个指针变量，要求这个指针变量指向对象必须同时遵守多个协议A，B
        ```objective-c
        NSObject<A,B> *obj = [Student new];
        id<A,B> *obj = [Student new];
        ```
    - 定义一个指针，指向遵守了A协议的学生对象
        ```
        Student<A> *stu = [Student new];
        ```
    - 为什么要进行协议的类型限制？
        - 遵守了某个协议的类，就相当于这个类拥有了这个协议所定义的行为。
        - 因为我要调用这个对象里的某些协议方法，所以我要限制这个类一定要遵守某协议，这个类中才会有协议方法。